...
function main()
{
	...
	if [ ! -n "${mdsnode}" ]; then
		print_message "MULTEXU ERROR" "the parameter --mdsnode is necessary..."
		exit 1;
	fi
}
...
if [ ! -n "${lmt_mgnode}" ]; then
	...
	do
		print_message "MULTEXU_INFO" "the current node is stalling cerebro..."
		sleep ${sleeptime}s
	done
	...
	while [[ cat ${EXECUTE_STATUS_SIGNAL} != "${MULTEXU_STATUS_EXECUTE}" ]];
	do
		print_message "MULTEXU_INFO" "the current node is stalling lmt-server..."
		sleep ${sleeptime}s
	done
	
	sh ${MULTEXU_BATCH_LMT_DIR}/_configure_cerebro_conf.sh
	...
	sh ${MULTEXU_BATCH_CRTL_DIR}/multexu.sh --iptable=${lmt_mgnode} --cmd="sh ${MULTEXU_BATCH_LMT_DIR}/_configure_cerebro_conf.sh"
fi
